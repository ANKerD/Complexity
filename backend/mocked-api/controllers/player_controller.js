...
module.exports.signup = async (req, res) => {
  ...
  if (p.length == 0) {
    try {
      ...
      await player.save();
      console.log(
        "Id from player [" + req.body.player.nick + "]: " + player._id
      );
      ...
      return res
        ...
        .send({ error: `${registration_failed_msg} due to ${error}` });
    }
  } else {
    res
      .status(httpStatusCode.BAD_REQUEST)
      .send({ error: user_exists_msg })
      .end();
  }
};
...
module.exports.login = async (req, res) => {
  ...
  try {
    ...
    if (!player) return res.status(400).send({ error: user_not_found_msg });
    const token = await player.generateAuthToken();
...
module.exports.profile = async (req, res) => {
  ...
  if (!otherPlayer) return res.status(400).send({ error: user_not_found_msg });
...
module.exports.forgetPassword = async (req, res) => {
  try {
    ...
    if (!player) return res.status(404).send({ error: user_not_found_msg });
    const { nick } = player;
